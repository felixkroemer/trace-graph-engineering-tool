package com.felixkroemer.trace_graph_engineering_tool.render.stateful;
...
public final class GraphRenderer {
    ...
    public static void renderEdges(ProgressMonitor pm, GraphGraphics grafx, CyNetworkViewSnapshot netView,
        ...
        if (flags.renderEdges() == GraphLOD.RenderEdges.ALL) {
        ...
        if (flags.not(LOD_HIGH_DETAIL)) { // Low detail.
            ...
            while (edgeHits.hasNext()) {
                ...
                Line2D line = new Line2D.Float(sourceNodeX, sourceNodeY, targetNodeX, targetNodeY);
            ...
            @SuppressWarnings("unchecked") View<CyNode>[] nodeBuff = new View[2];
            ...
            while (edgeHits.hasNext()) {
                ...
                Line2D.Float line = new Line2D.Float(sourceNodeX, sourceNodeY, targetNodeX, targetNodeY);
                if (!line.intersects(area)) {
                    continue;
                }
                ...
                if (flags.not(LOD_EDGE_ARROWS) || stacking == EdgeStackingVisualProperty.HAYSTACK) { // Not rendering
                    // arrows.
                ...
                if (stacking == EdgeStackingVisualProperty.HAYSTACK) {
                    ...
                    computeEdgeEndpointsHaystack(floatBuff1, floatBuff2, srcSuid, trgSuid, edgeInfo.getSUID(),
                            radiusModifier, stacking, floatBuff3, floatBuff4, haystackDataBuff);
                } else /* auto bend */ {
                    computeEdgeEndpoints(floatBuff1, srcShape, srcArrow, srcArrowSize, anchors, floatBuff2, trgShape,
                ...
                grafx.drawEdgeFull(srcArrow, srcArrowSize, srcArrowPaint, trgArrow, trgArrowSize, trgArrowPaint,
                        srcXAdj, srcYAdj, anchors, trgXAdj, trgYAdj, thickness, edgeStroke, segPaint);
                    ...
                    for (int k = 0; k < anchors.numAnchors(); k++) {
                        ...
                        if ((anchorSize = edgeDetails.getAnchorSize(edge, k)) > 0.0f) {
                            ...
                            grafx.drawNodeFull(GraphGraphics.SHAPE_RECTANGLE,
                                    ...
                                    (float) (floatBuff4[0] + (anchorSize / 2.0d)),
                                    (float) (floatBuff4[1] + (anchorSize / 2.0d)), edgeDetails.getAnchorPaint(edge,
                                            k), 0.0f, null, null);
                        }
                    }
                }
                ...
                if (flags.has(LOD_EDGE_LABELS) && !(stacking == EdgeStackingVisualProperty.HAYSTACK)) {
                    ...
                    for (int labelInx = 0; labelInx < labelCount; labelInx++) {
                        ...
                        renderText(grafx, labelInfo, (float) textXCenter, (float) textYCenter, textXCenter,
                                textYCenter, justify, paint, backgroundPaint, backgroundShape, theta,
    ...
    public static void getEdgeLabelPosition(View<CyEdge> edge, EdgeDetails edgeDetails, RenderDetailFlags flags,
                                            LabelInfo labelInfo, float[] floatBuff3, float[] floatBuff4,
                                            EdgeAnchors anchors, double[] offsetBuff, double[] returnBuff) {
        ...
        if (edgeAnchor == Position.NORTH || edgeAnchor == Position.NORTH_WEST || edgeAnchor == Position.NORTH_EAST) {
            // We sort of "fake" this by adding the edge width to the offsetVectorY.  That way, any trigonometry only
            // needs
            // to get done once
        ...
        if (edgeAnchor == Position.WEST || edgeAnchor == Position.SOUTH_WEST || edgeAnchor == Position.NORTH_WEST) {
            ...
            if ((numPaths % 2) != 0) {
                ...
                } else throw new IllegalStateException("got unexpected PathIterator segment type: " + subPathType);
            } else { // numPaths % 2 == 0.
                ...
                if (subPathType == PathIterator.SEG_LINETO) {
                    ...
                    edgeAnchorPointX =
                            (0.125d * floatBuff5[6]) + (0.375d * floatBuff5[0]) + (0.375d * floatBuff5[2]) + (0.125d * floatBuff5[4]);
                    edgeAnchorPointY =
                            (0.125d * floatBuff5[7]) + (0.375d * floatBuff5[1]) + (0.375d * floatBuff5[3]) + (0.125d * floatBuff5[5]);
                } else throw new IllegalStateException("got unexpected PathIterator segment type: " + subPathType);
            }
        } else throw new IllegalStateException("encountered an invalid EDGE_ANCHOR_* constant: " + edgeAnchor);
        ...
        // System.out.println("offsetVectorX,Y="+offsetVectorX+","+offsetVectorY+" offsetBuff="+offsetBuff[0]+",
        // "+offsetBuff[1]);
    ...
    public static void reverseOffset(double xOffset, double yOffset, double slope, double lineAngle, double[] xy) {
        ...
        if (determinant != 0) {
            ...
            // Deal with our "flipping"
            double deg = Math.toDegrees(lineAngle);
            if (deg > 0 && deg < 90) xy[1] = -xy[1];
            if (deg < -90 && deg > -180) xy[1] = -xy[1];
            return;
        }
    ...
    public static void updateOffset(double xOffset, double yOffset, double slope, double lineAngle, double[] xy) {
        ...
        double deg = Math.toDegrees(lineAngle);
        if (deg > 0 && deg < 90) yOffset = -yOffset;
        if (deg < -90 && deg > -180) yOffset = -yOffset;
    ...
    public static void renderNodes(ProgressMonitor pm, GraphGraphics grafx, CyNetworkViewSnapshot netView,
                                   RenderDetailFlags flags, NodeDetails nodeDetails,
                                   Set<VisualPropertyDependency<?>> dependencies, LabelInfoProvider labelInfoProvider) {
        ...
        Rectangle2D.Float area = grafx.getTransform().getNetworkVisibleAreaNodeCoords();
        NodeSpacialIndex2DEnumerator nodeHits = netView.getSpacialIndex2D().queryOverlapNodes(area.x, area.y,
                area.x + area.width, area.y + area.height, pm::isCancelled);
        ...
        if (flags.not(LOD_HIGH_DETAIL)) { // Low detail.
            ...
            while (nodeHits.hasNext()) {
                ...
                renderNodeHigh(netView, grafx, node, floatBuff1, doubleBuff1, doubleBuff2, nodeDetails, flags,
                        dependencies);
                ...
                if (flags.has(LOD_NODE_LABELS)) { // Potential label rendering.
                    ...
                    if (text != null && !text.isEmpty()) {
                        ...
                        LabelInfo labelInfo = labelInfoProvider.getLabelInfo(text, font, nodeLabelWidth,
                                grafx.getFontRenderContextFull());
                        ...
                        renderText(grafx, labelInfo, (float) textXCenter, (float) textYCenter,
                                nodeAnchorPointX + offsetVectorX, nodeAnchorPointY + offsetVectorY, justify, paint,
                                backgroundPaint, backgroundShape, theta, flags.has(LOD_TEXT_AS_SHAPE));
    ...
    public final static void renderText(final GraphGraphics grafx, final LabelInfo measuredText,
                                        final float textXCenter, final float textYCenter, final double textXAnchor,
                                        final double textYAnchor, final Justification textJustify, final Paint paint,
                                        final Paint backgroundPaint, final byte backgroundShape, final double theta,
        ...
        for (LabelLineInfo line : measuredText.getMeasuredLines()) {
            ...
            if (textJustify == Justification.JUSTIFY_CENTER) xCenter = textXCenter;
                ...
                xCenter = (0.5d * (overallWidth - line.getWidth())) + textXCenter;
            else throw new IllegalStateException("textJustify value unrecognized");

            grafx.drawTextFull(line, (float) xCenter, (float) yCenter, textXAnchor, textYAnchor, (float) theta, paint
                    , backgroundPaint, backgroundShape, textAsShape);
    ...
    public final static void computeEdgeEndpointsHaystack(float[] srcNodeExtents, float[] trgNodeExtents,
     ...
     * @return DOCUMENT ME!
     */
    public final static void computeEdgeEndpoints(final float[] srcNodeExtents, final byte srcNodeShape,
                                                  final ArrowShape srcArrow, final float srcArrowSize,
                                                  EdgeAnchors anchors, final float[] trgNodeExtents,
                                                  final byte trgNodeShape, final ArrowShape trgArrow,
                                                  final float trgArrowSize, final float[] rtnValSrc,
    ...
    private static void calcIntersection(byte nodeShape, float[] nodeExtents, float x, float y, float xOut,
                                         float yOut, float[] retVal) {
        if ((nodeExtents[0] == nodeExtents[2]) || (nodeExtents[1] == nodeExtents[3])) {
            retVal[0] = x;
            retVal[1] = y;
        } else {
            if (!GraphGraphics.computeEdgeIntersection(nodeShape, nodeExtents[0], nodeExtents[1], nodeExtents[2],
                    nodeExtents[3], 0.0f, xOut, yOut, retVal)) {
                ...
                final double desiredDist = Math.max(((double) nodeExtents[2]) - nodeExtents[0],
                        ((double) nodeExtents[3]) - nodeExtents[1]);
                ...
                GraphGraphics.computeEdgeIntersection(nodeShape, nodeExtents[0], nodeExtents[1], nodeExtents[2],
                        nodeExtents[3], 0.0f, newXOut, newYOut, retVal);
            }
        }
    }
        ...
        synchronized (s_floatTemp) {
            ...
            for (int i = 0; i < segs; i++) {
                switch (s_segTypeBuff[i]) {
                    ...
                    case PathIterator.SEG_QUADTO:
                        rtnVal.quadTo(s_floatBuff2[offset++], s_floatBuff2[offset++], s_floatBuff2[offset++],
                                s_floatBuff2[offset++]);

                        break;

                    default: // PathIterator.SEG_CUBICTO.
                        rtnVal.curveTo(s_floatBuff2[offset++], s_floatBuff2[offset++], s_floatBuff2[offset++],
                                s_floatBuff2[offset++], s_floatBuff2[offset++], s_floatBuff2[offset++]);
            ...
            for (int i = segs - 1; i > 0; i--) {
                switch (s_segTypeBuff[i]) {
                    ...
                    case PathIterator.SEG_QUADTO:
                        offset -= 4;
                        rtnVal.quadTo(s_floatBuff2[offset], s_floatBuff2[offset + 1], s_floatBuff2[offset - 2],
                                s_floatBuff2[offset - 1]);

                        break;

                    default: // PathIterator.SEG_CUBICTO.
                        offset -= 6;
                        rtnVal.curveTo(s_floatBuff2[offset + 2], s_floatBuff2[offset + 3], s_floatBuff2[offset],
                                s_floatBuff2[offset + 1], s_floatBuff2[offset - 2], s_floatBuff2[offset - 1]);
    ...
    static final boolean _computeEdgeIntersection(final float nodeX, final float nodeY, final float offset,
                                                  final float ptX, final float ptY, final boolean alwaysCompute,
    ...
    private static final void renderNodeHigh(final CyNetworkViewSnapshot netView, final GraphGraphics grafx,
                                             final View<CyNode> cyNode, final float[] floatBuff1,
                                             final double[] doubleBuff1, final double[] doubleBuff2,
                                             final NodeDetails nodeDetails, final RenderDetailFlags flags,
        ...
        if ((floatBuff1[0] != floatBuff1[2]) && (floatBuff1[1] != floatBuff1[3])) {
            ...
            if (flags.not(LOD_NODE_BORDERS)) { // Not rendering borders.
                ...
                if (borderWidth == 0.0f) borderPaint = null;
                else borderPaint = nodeDetails.getBorderPaint(cyNode);
            }
        ...
        if (flags.has(LOD_CUSTOM_GRAPHICS)) {
            ...
            if (nestedNetworkPaint != null) {
                ...
                grafx.drawCustomGraphicImage(nestedNetworkPaint.getAnchorRect(), (float) doubleBuff2[0],
                        (float) doubleBuff2[1], nestedNetworkPaint);
            }
            ...
            synchronized (nodeDetails) {
                ...
                final Map<VisualProperty<CyCustomGraphics>, CustomGraphicsInfo> cgMap =
                        nodeDetails.getCustomGraphics(cyNode);
                if (cgMap != null) {
                    final List<CustomGraphicsInfo> infoList = new ArrayList<>(cgMap.values());

                    // MKTODO I guess there's no way around doing this? The charts need access to the underlying
                    // table model.
                    ...
                    if (mutableNode != null) {
                        for (CustomGraphicsInfo cgInfo : infoList) {
                            List<CustomGraphicLayer> layers = cgInfo.createLayers(netViewForCharts, mutableNode,
                            ...
                            for (CustomGraphicLayer layer : layers) {
                                ...
                                grafx.drawCustomGraphicFull(netViewForCharts, mutableNode, nodeShape, layer, xOffset,
                                        yOffset);
                            }
                        }
                    }
