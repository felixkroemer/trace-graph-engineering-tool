...
public class RenderDetailFlags {
    ...
    public static GraphLOD.RenderEdges renderEdges(CyNetworkViewSnapshot netView, NetworkTransform transform,
												   GraphLOD lod) {
        Rectangle2D.Float area = transform.getNetworkVisibleAreaNodeCoords();
        SpacialIndex2DEnumerator<Long> nodeHits = netView.getSpacialIndex2D().queryOverlap(area.x, area.y,
    ...
    public static RenderDetailFlags create(CyNetworkViewSnapshot netView, NetworkTransform transform, GraphLOD lod,
										   DRenderingEngine.UpdateType updateType) {
        Rectangle2D.Float area = transform.getNetworkVisibleAreaNodeCoords();
        SpacialIndex2DEnumerator<Long> nodeHits = netView.getSpacialIndex2D().queryOverlap(area.x, area.y,
				area.x + area.width, area.y + area.height);
        final int visibleNodeCount = nodeHits.size();
    ...
    public static RenderDetailFlags create(CyNetworkViewSnapshot netView, int visibleNodeCount, GraphLOD lod) {
        return create(netView, visibleNodeCount, lod, DRenderingEngine.UpdateType.ALL_FULL);
    }


    private static RenderDetailFlags create(CyNetworkViewSnapshot netView, int visibleNodeCount, GraphLOD lod,
											DRenderingEngine.UpdateType updateType) {
        final int totalNodeCount = netView.getNodeCount();
        final int totalEdgeCount = netView.getEdgeCount();
        final GraphLOD.RenderEdges renderEdges = lod.renderEdges(visibleNodeCount, totalNodeCount, totalEdgeCount);

        int renderEdgeCount;
        if (renderEdges == GraphLOD.RenderEdges.ALL) renderEdgeCount = totalEdgeCount;
        else if (renderEdges == GraphLOD.RenderEdges.NONE) renderEdgeCount = 0;
        else // visible nodes
    ...
    private static int lodToBits(CyNetworkViewSnapshot netView, int renderNodeCount, int renderEdgeCount,
								 GraphLOD lod, DRenderingEngine.UpdateType updateType) {
        int lodbits = 0;
        ...
        if (highDetail) {
            ...
            if (lod.nodeBorders(renderNodeCount, renderEdgeCount)) lodbits |= LOD_NODE_BORDERS;
            if (lod.nodeLabels(renderNodeCount, renderEdgeCount)) lodbits |= LOD_NODE_LABELS;
            if (lod.edgeArrows(renderNodeCount, renderEdgeCount)) lodbits |= LOD_EDGE_ARROWS;
            if (lod.dashedEdges(renderNodeCount, renderEdgeCount)) lodbits |= LOD_DASHED_EDGES;
            if (lod.edgeAnchors(renderNodeCount, renderEdgeCount)) lodbits |= LOD_EDGE_ANCHORS;
            if (lod.edgeLabels(renderNodeCount, renderEdgeCount)) lodbits |= LOD_EDGE_LABELS;
            ...
            if (lod.customGraphics(renderNodeCount, renderEdgeCount)) lodbits |= LOD_CUSTOM_GRAPHICS;
        }

        // optimization bits
        if (lod.isEdgeBufferPanEnabled()) lodbits |= OPT_EDGE_BUFF_PAN;
        if (lod.isLabelCacheEnabled()) lodbits |= OPT_LABEL_CACHE;
