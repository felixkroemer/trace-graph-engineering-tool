package com.felixkroemer.trace_graph_engineering_tool.renderer.graph.render.immed;

import com.felixkroemer.trace_graph_engineering_tool.renderer.ding.impl.canvas.GraphicsProvider;
import com.felixkroemer.trace_graph_engineering_tool.renderer.ding.impl.canvas.NetworkTransform;
import com.felixkroemer.trace_graph_engineering_tool.renderer.graph.render.immed.arrow.*;
import com.felixkroemer.trace_graph_engineering_tool.renderer.graph.render.immed.nodeshape.*;
import com.felixkroemer.trace_graph_engineering_tool.renderer.graph.render.stateful.LabelLineInfo;
...
public final class GraphGraphics {
    ...
    // A constant for controlling how cubic Bezier curves are drawn; This particular constant results in
    // elliptical-looking curves.
     ...
     * @throws IllegalArgumentException if xMin is not less than xMax, if yMin is not less than
     ...
     * previously.
     *
     * @param nodeShape   the shape of the node to draw (one of the SHAPE_* constants or
     *                    a custom node shape).
     * @param xMin        an extent of the node shape to draw, in node coordinate space;
     *                    the drawn shape will theoretically contain a point that lies
     *                    on this X coordinate.
     * @param yMin        an extent of the node shape to draw, in node coordinate space;
     *                    the drawn shape will theoretically contain a point that lies
     *                    on this Y coordinate.
     * @param xMax        an extent of the node shape to draw, in node coordinate space;
     *                    the drawn shape will theoretically contain a point that lies
     *                    on this X coordinate.
     * @param yMax        an extent of the node shape to draw, in node coordinate space;
     *                    the drawn shape will theoretically contain a point that lies
     *                    on this Y coordinate.
     * @param fillPaint   the paint to use when drawing the node area minus the border
     *                    (the "interior" of the node).
     * @param borderWidth the border width, in node coordinate space; if this value is
     *                    zero, the rendering engine skips over the process of rendering
     *                    the border, which gives a significant performance boost.
     * @param borderPaint if borderWidth is not zero, this paint is used for rendering
     *                    the node border; otherwise, this parameter is ignored (and may
     *                    be null).
     * @throws IllegalArgumentException if xMin is not less than xMax or if yMin is not less than
     ...
     */
    public final Shape drawNodeFull(final byte nodeShape, final float xMin, final float yMin, final float xMax,
                                    final float yMax, final Paint fillPaint, final float borderWidth,
                                    final Stroke borderStroke, final Paint borderPaint) {
        if (debug) {
            checkOrder(xMin, xMax, "x");
            checkOrder(yMin, yMax, "y");
            if (!(borderWidth >= 0.0f)) {
        ...
        if (borderWidth > 0.0f) {
            ...
            if (borderStroke != null) m_g2d.setStroke(borderStroke);
            else m_g2d.setStroke(getStroke(borderWidth));
     ...
     *
     * @param nodeShape the shape (SHAPE_* constant or custom shape) of the node in
     *                  question.
     ...
     * @param path      the computed path is returned in this parameter; the computed
     ...
     *
     * @param coords      vertexCount * 2 consecutive coordinate values are read from
     ...
     *                    specified.
     * @param offset      the starting index of where to read coordinates from in the
     *                    coords parameter.
     * @param vertexCount the number of vertices to read from coords; vertexCount * 2
     ...
     * @throws IllegalArgumentException if any of the constraints are not met, or if the specified
     *                                  polygon has more than CUSTOM_SHAPE_MAX_VERTICES vertices.
     * @throws IllegalStateException    if too many custom node shapes are already defined; a
    ...
    public final byte defineCustomNodeShape(final float[] coords, final int offset, final int vertexCount) {
        ...
        if (xDist == 0.0d) throw new IllegalArgumentException("polygon does not move in the X direction");

        final double yDist = yMax - yMin;

        if (yDist == 0.0d) throw new IllegalArgumentException("polygon does not move in the Y direction");
        ...
        if (nextCustomShapeType < 0) throw new IllegalStateException("too many custom node shapes are already defined");
    ...
    public final float[] getCustomNodeShape(final byte customShape) {
        if (!customNodeShapeExists(customShape)) return null;
     ...
     *
     * @param grafx custom node shapes will be imported from this GraphGraphics.
     * @throws IllegalStateException if at least one custom node shape is already defined in
     *                               this GraphGraphics.
     */
     ...
     * @throws IllegalArgumentException if edgeColor is not opaque.
     */
    public final void drawEdgeLow(final float x0, final float y0, final float x1, final float y1,
     ...
     *
     * @param arrow0Type    the type of arrow shape to use for drawing the arrow at point
     *                      (x0, y0); this value must be one of the ARROW_* constants.
     * @param arrow0Size    the size of arrow at point (x0, y0); how size is interpreted
     *                      for different arrow types is described in the table above.
     * @param arrow0Paint   the paint to use when drawing the arrow at point (x0, y0).
     * @param arrow1Type    the type of arrow shape to use for drawing the arrow at point
     *                      (x1, y1); this value must be one of the ARROW_* constants.
     * @param arrow1Size    the size of arrow at point (x1, y1); how size is interpreted
     *                      for different arrow types is described in the table above.
     * @param arrow1Paint   the paint to use when drawing the arrow at point (x1, y1).
     * @param x0            the X coordinate of the first edge endpoint.
     * @param y0            the Y coordinate of the first edge endpoint.
     * @param anchors       anchor points between the two edge endpoints; null is an
     ...
     * @param edgeThickness the thickness of the edge segment; the edge segment is the
     *                      part of the edge between the two endpoint arrows.
     * @param edgeStroke    the Stroke to use when drawing the edge segment.
     * @param edgePaint     the paint to use when drawing the edge segment.
     * @throws IllegalArgumentException if edgeThickness is less than zero, if any one of the arrow
     *                                  configurations does not meet specified criteria, or if more
     *                                  than MAX_EDGE_ANCHORS anchors are specified.
     */
    public final void drawEdgeFull(final ArrowShape arrow0Type, final float arrow0Size, final Paint arrow0Paint,
                                   final ArrowShape arrow1Type, final float arrow1Size, final Paint arrow1Paint,
                                   final float x0, final float y0, EdgeAnchors anchors, final float x1,
                                   final float y1, final float edgeThickness, Stroke edgeStroke,
                                   final Paint edgePaint) {
        final double curveFactor = CURVE_ELLIPTICAL;
        final boolean simpleSegment =
                arrow0Type == ArrowShapeVisualProperty.NONE && arrow1Type == ArrowShapeVisualProperty.NONE;
        ...
        if (!simpleSegment && edgeStroke instanceof BasicStroke && ((BasicStroke) edgeStroke).getEndCap() != BasicStroke.CAP_BUTT) {
            ...
            edgeStroke = new BasicStroke(bs.getLineWidth(), BasicStroke.CAP_BUTT, bs.getLineJoin(),
        ...
        final int edgePtsCount = computeCubicPolyEdgePath(m_edgePtsBuff, m_floatBuff, arrow0Type,
                ...
                (arrow1Type == ArrowShapeVisualProperty.NONE) ? 0.0f : arrow1Size, x0, y0, anchors, x1, y1,
                curveFactor);

        if (edgePtsCount < 3) {
            if (edgePtsCount == 2) { // Draw an ordinary edge.
                drawSimpleEdgeFull(arrow0Type, arrow0Size, arrow0Paint, arrow1Type, arrow1Size, arrow1Paint,
                        (float) m_edgePtsBuff[0], (float) m_edgePtsBuff[1], (float) m_edgePtsBuff[2],
                        (float) m_edgePtsBuff[3], edgeThickness, edgeStroke, edgePaint);
            }
            return;
        }
        ...
        while (inx < count) {
            m_path2d.curveTo((float) m_edgePtsBuff[inx++], (float) m_edgePtsBuff[inx++], (float) m_edgePtsBuff[inx++]
                    , (float) m_edgePtsBuff[inx++], (float) m_edgePtsBuff[inx++], (float) m_edgePtsBuff[inx++]);
        }
        ...
        if (edgeStroke instanceof BasicStroke) {
            ...
            if (arrow0Cap != null) {
                m_xformUtil.setTransform(cosTheta0, sinTheta0, -sinTheta0, cosTheta0, m_edgePtsBuff[2],
                        m_edgePtsBuff[3]);
            ...
            if (arrow1Cap != null) {
                m_xformUtil.setTransform(cosTheta1, sinTheta1, -sinTheta1, cosTheta1,
                        m_edgePtsBuff[((edgePtsCount - 1) * 6) - 4], m_edgePtsBuff[((edgePtsCount - 1) * 6) - 3]);
                m_g2d.transform(m_xformUtil);
                m_g2d.scale(edgeThickness, edgeThickness);
                // The paint is already set to edge paint.
    ...
    private final void edgeFullDebug(final ArrowShape arrow0Type, final float arrow0Size, final ArrowShape arrow1Type
            , float arrow1Size, final Stroke edgeStroke, final float edgeThickness, final EdgeAnchors anchors) {
        if (!(edgeThickness >= 0.0f)) {
            throw new IllegalArgumentException("edgeThickness < 0");
        }

        if (!arrows.containsKey(arrow0Type)) throw new IllegalArgumentException("arrow0Type is not recognized");

        if (arrow0Type != ArrowShapeVisualProperty.NONE) if (!(arrow0Size >= edgeThickness))
            throw new IllegalArgumentException("arrow size must be at least as large as edge thickness");

        if (!arrows.containsKey(arrow1Type)) throw new IllegalArgumentException("arrow1Type is not recognized");

        if (arrow1Type != ArrowShapeVisualProperty.NONE) if (!(arrow1Size >= edgeThickness))
            throw new IllegalArgumentException("arrow size must be at least as large as edge thickness");

        if (anchors.numAnchors() > MAX_EDGE_ANCHORS) {
            throw new IllegalArgumentException("at most MAX_EDGE_ANCHORS (" + MAX_EDGE_ANCHORS + ") edge anchors can "
                    + "be specified");
        }
    }

    private final void drawSimpleEdgeFull(final ArrowShape arrow0Type, final float arrow0Size,
                                          final Paint arrow0Paint, final ArrowShape arrow1Type,
                                          final float arrow1Size, final Paint arrow1Paint, final float x0,
                                          final float y0, final float x1, final float y1, final float edgeThickness,
                                          final Stroke edgeStroke, final Paint edgePaint) {
        final double len =
                Math.sqrt(((((double) x1) - x0) * (((double) x1) - x0)) + ((((double) y1) - y0) * (((double) y1) - y0)));
        ...
        if (arrow0 != null) {
            ...
            if (filled) m_g2d.fill(arrow0);
                ...
                m_g2d.setStroke(new BasicStroke(0.25f));
                if (arrow0Type == ArrowShapeVisualProperty.OPEN_CIRCLE) m_g2d.setStroke(new BasicStroke(0.1f));
                m_g2d.draw(arrow0);
            }
            m_g2d.setTransform(m_currNativeXform);
        }
        ...
        if (arrow1 != null) {
            ...
            if (filled) m_g2d.fill(arrow1);
                ...
                m_g2d.setStroke(new BasicStroke(0.25f));
                if (arrow1Type == ArrowShapeVisualProperty.OPEN_CIRCLE) m_g2d.setStroke(new BasicStroke(0.1f));
                m_g2d.draw(arrow1);
            }
            m_g2d.setTransform(m_currNativeXform);
        }
     ...
     *
     * @param arrow0Type the type of arrow shape used for drawing the arrow at point
     *                   (x0, y0); this value must be one of the ARROW_* constants.
     * @param arrow0Size the size of arrow at point (x0, y0).
     * @param arrow1Type the type of arrow shape used for drawing the arrow at point
     *                   (x1, y1); this value must be one of the ARROW_* constants.
     ...
     * @throws IllegalArgumentException if any one of the edge arrow criteria specified in
    ...
    public static boolean getEdgePath(ArrowShape arrow0Type, float arrow0Size, ArrowShape arrow1Type,
                                      float arrow1Size, float x0, float y0, EdgeAnchors anchors, float x1, float y1,
                                      GeneralPath path) {
        final double curveFactor = CURVE_ELLIPTICAL;
        ...
        int edgePtsCount = computeCubicPolyEdgePath(edgePtsBuff, floatBuff, arrow0,
                (arrow0 == ArrowShapeVisualProperty.NONE) ? 0.0f : arrow0Size, arrow1,
                (arrow1 == ArrowShapeVisualProperty.NONE) ? 0.0f : arrow1Size, x0, y0, anchors, x1, y1, curveFactor);
        ...
        while (inx < count) {
            path.curveTo((float) edgePtsBuff[inx++], (float) edgePtsBuff[inx++], (float) edgePtsBuff[inx++],
                    (float) edgePtsBuff[inx++], (float) edgePtsBuff[inx++], (float) edgePtsBuff[inx++]);
        }
     ...
     */
    private static int computeCubicPolyEdgePath(double[] edgePtsBuff, float[] floatBuff, ArrowShape arrow0Type,
                                                float arrow0Size, ArrowShape arrow1Type, float arrow1Size, float x0,
                                                float y0, EdgeAnchors anchors, float x1, float y1, double curveFactor) {
        ...
        while (edgePtsCount > 1) {
            ...
            if ((edgePtsBuff[edgePtsCount * 2 - 2] == edgePtsBuff[edgePtsCount * 2 - 4]) && (edgePtsBuff[edgePtsCount * 2 - 1] == edgePtsBuff[edgePtsCount * 2 - 3])) {
                edgePtsCount--;
            } else {
                break;
            }
        ...
        // 6 represents the offset in the buffer.
        {
            ...
            edgePtsBuff[edgePtsCount * 6 - 4] =
                    edgePtsBuff[edgePtsCount * 6 - 2] + (dx * arrow1Size * getT(arrow1Type));
            edgePtsBuff[edgePtsCount * 6 - 3] =
                    edgePtsBuff[edgePtsCount * 6 - 1] + (dy * arrow1Size * getT(arrow1Type));
            ...
            double candX2 =
                    edgePtsBuff[edgePtsCount * 6 - 4] + (curveFactor * (edgePtsBuff[edgePtsCount * 2 - 2] - edgePtsBuff[edgePtsCount * 6 - 4]));
            ...
            double candY2 =
                    edgePtsBuff[edgePtsCount * 6 - 3] + (curveFactor * (edgePtsBuff[edgePtsCount * 2 - 1] - edgePtsBuff[edgePtsCount * 6 - 3]));
     ...
     * twice the minimum of the width and height of the node.
     *
     * @param nodeShape the shape of the node in question; this must be one of the
     *                  SHAPE_* constants or a custom node shape.
     * @param xMin      an extent of the node in question, in node coordinate space.
     * @param yMin      an extent of the node in question, in node coordinate space.
     * @param xMax      an extent of the node in question, in node coordinate space.
     * @param yMax      an extent of the node in question, in node coordinate space.
     * @param offset    most of the time this value will be zero, in which case the
     ...
     *                  node outline.
     * @param ptX       specifies the X coordinate of the endpoint of the line segment
     *                  that is not the endpoint lying in the center of the node
     *                  shape.
     * @param ptY       specifies the Y coordinate of the endpoint of the line segment
     *                  that is not the endpoint lying in the center of the node
     *                  shape.
     * @param returnVal if true is returned, returnVal[0] is set to be the X
    ...
    public static final boolean computeEdgeIntersection(byte nodeShape, float xMin, float yMin, float xMax,
                                                        float yMax, float offset, float ptX, float ptY,
                                                        float[] returnVal) {
        NodeShape ns = nodeShapes.get(nodeShape);
     ...
     * @param scaleFactor     in order to prevent very small fonts from "degenerating" it is
     ...
     * @param xCenter         the text string is drawn such that its logical bounds
     ...
     * @param drawTextAsShape this flag controls the way that text is drawn to the
    ...
    public final void drawTextFull(LabelLineInfo labelLineInfo, final float xCenter, final float yCenter,
                                   final double xAnchor, final double yAnchor, final float theta, final Paint paint,
    ...
    private void drawLabelBackground(Rectangle2D bounds, Paint paint, byte shape) {
        if (shape >= 0) {
            ...
            float padding = 0.0f;
            Shape bgs = getShape(shape, (float) bounds.getMinX() - padding, (float) bounds.getMinY() - padding,
     ...
     * @param cg        the CustomGraphicLayer
     * @param xOffset   in node coordinates, a value to add to the X coordinates of
     *                  the shape's definition.
     * @param yOffset   in node coordinates, a value to add to the Y coordinates of
     *                  the shape's definition.
     */
    public final void drawCustomGraphicFull(CyNetworkView netView, View<CyNode> node, Shape nodeShape,
                                            CustomGraphicLayer cg, float xOffset, float yOffset) {
        m_g2d.translate(xOffset, yOffset);
    ...
    private void checkOrder(float min, float max, String id) {
        if (!(min < max)) throw new IllegalArgumentException(id + "Min not less than " + id + "Max");
    }
