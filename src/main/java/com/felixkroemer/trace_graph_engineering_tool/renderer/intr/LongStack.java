 ...
 * class versioning.
 */
public final class LongStack implements java.io.Serializable {
     ...
     */
    public LongStack() {
        m_stack = new long[DEFAULT_CAPACITY];
        empty();
    }
    ...
    private final void checkSize() {
        ...
        final int newStackSize = (int) Math.min((long) Integer.MAX_VALUE, (((long) m_stack.length) * 2L) + 1L);

        if (newStackSize == m_stack.length) throw new IllegalStateException("cannot allocate large enough array");

        final long[] newStack = new long[newStackSize];
