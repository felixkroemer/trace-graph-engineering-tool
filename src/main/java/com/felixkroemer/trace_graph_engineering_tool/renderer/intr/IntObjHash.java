...
public final class IntObjHash implements java.io.Serializable {
    ...
    public final Object put(final int key, final Object value) {
        ...
        if (key != m_prevKey) {
            ...
            for (m_prevInx = key % m_keys.length; (m_keys[m_prevInx] >= 0) && (m_keys[m_prevInx] != key); m_prevInx =
					(m_prevInx + incr) % m_keys.length)
    ...
    private final void incrSize() {
        ...
        for (int i = 0; i < m_elements; i++) {
            ...
            for (newIndex = m_keys[oldIndex] % newKeys.length; newKeys[newIndex] >= 0; newIndex =
					(newIndex + incr) % newKeys.length)
