...
public final class LongLongHash implements java.io.Serializable {
    private final static long serialVersionUID = 121374594943384L;
     ...
     * complexity O(1).
     *
     * @throws IllegalArgumentException if either key or value is negative.
     */
    public final long put(final long key, final long value) {
        ...
        if (key != m_prevKey) {
            ...
            for (m_prevInx = (int) (key % m_size); (m_keys[m_prevInx] >= 0) && (m_keys[m_prevInx] != key); m_prevInx
					= (m_prevInx + incr) % m_size)
                if (incr == 0) incr = 1 + (int) (key % (m_size - 1));
    ...
    private final void incrSize() {
        ...
        for (int i = 0; i < m_elements; i++) {
            ...
            for (newIndex = (int) (m_keyDump[oldIndex] % m_size); m_keys[newIndex] >= 0; newIndex =
					(newIndex + incr) % m_size)
                if (incr == 0) incr = 1 + (int) (m_keyDump[oldIndex] % (m_size - 1));
