 ...
 * serializable.
 */
public final class LongObjHash implements java.io.Serializable {
    ...
    public final Object put(final long key, final Object value) {
        ...
        if (key != m_prevKey) {
            ...
            for (m_prevInx = (int) (key % (long) m_keys.length); (m_keys[m_prevInx] >= 0) && (m_keys[m_prevInx] != key); m_prevInx = (int) ((m_prevInx + incr) % (long) m_keys.length))
                if (incr == 0) incr = 1 + (int) (key % (long) (m_keys.length - 1));
    ...
    private final void incrSize() {
        ...
        for (int i = 0; i < m_elements; i++) {
            ...
            for (newIndex = (int) (m_keys[oldIndex] % (long) newKeys.length); newKeys[newIndex] >= 0; newIndex =
					(int) ((newIndex + incr) % (long) newKeys.length))
                if (incr == 0) incr = 1 + (int) (m_keys[oldIndex] % (long) (newKeys.length - 1));
