 ...
 * class versioning.
 */
public final class MinLongHeap implements java.io.Serializable {
     ...
     */
    public MinLongHeap() {
        m_heap = new long[DEFAULT_CAPACITY + 1];
     ...
     * @throws IndexOutOfBoundsException if the combination of the
    ...
    public final void toss(long[] elements, int beginIndex, int length) {
        ...
        if (beginIndex < 0) throw new IndexOutOfBoundsException("beginIndex is less than zero");

        if (length < 0) throw new IndexOutOfBoundsException("length is less than zero");
    ...
    public final long deleteMin() {
        ...
        final long returnThis = m_heap[1];
    ...
    private static final void percolateDown(long[] heap, int parentIndex, int size) {
        ...
        for (int childIndex = parentIndex * 2; (childIndex <= size) && (childIndex > 0); // Check for overflow.
            ...
            if (((childIndex + 1) <= size) && (heap[childIndex + 1] < heap[childIndex])) childIndex++;

            if (heap[childIndex] < heap[parentIndex]) swap(heap, parentIndex, childIndex);
            else
    ...
    public final LongEnumerator orderedElements(boolean pruneDuplicates) {
        ...
        if (pruneDuplicates) {
            ...
            return new LongEnumerator() {
                ...
                long m_prevValue = heap[m_index] + 1;
