...
public final class IntIntHash implements java.io.Serializable {
    ...
    public final int put(final int key, final int value) {
        ...
        if (key != m_prevKey) {
            ...
            for (m_prevInx = key % m_size; (m_keys[m_prevInx] >= 0) && (m_keys[m_prevInx] != key); m_prevInx =
    ...
    private final void incrSize() {
        ...
        for (int i = 0; i < m_elements; i++) {
            ...
            for (newIndex = m_keyDump[oldIndex] % m_size; m_keys[newIndex] >= 0; newIndex = (newIndex + incr) % m_size)
