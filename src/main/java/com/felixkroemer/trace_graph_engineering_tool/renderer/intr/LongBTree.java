 ...
 * class versioning.
 */
public final class LongBTree implements java.io.Serializable {
     ...
     * @throws IllegalArgumentException if maxBranches is less than three.
     */
    public LongBTree(final int maxBranches) {
     ...
     * levels".)
     */
    private final static Node insert(final Node n, final long x, final int maxBranches) {
        if (isLeafNode(n)) {
     ...
     *                 +---+---+---+---+---+---+---+
     */
    private final static void split(final long newVal, final long[] origBuff, final long[] overflowBuff,
									final int overflowCount) {
        long[] currentArr = overflowBuff;
     ...
     */
    public final boolean delete(final long x) {
        final boolean returnThis = delete(m_root, x, m_minBranches);

        if ((!isLeafNode(m_root)) && (m_root.sliceCount == 1)) m_root = m_root.data.children[0];

        return returnThis;
    }

    private final static boolean delete(final Node n, final long x, final int minBranches) {
        if (isLeafNode(n)) {
            int foundInx = -1;
            ...
            for (int i = n.sliceCount - 2; i >= -1; i--) {
                long currentMin = ((i < 0) ? Integer.MIN_VALUE : n.data.splitVals[i]);
            ...
            if (affectedChild.sliceCount < minBranches) { // Underflow handling.
                ...
                final Node rightChild = ((deletedPath + 1) < n.sliceCount) ? n.data.children[deletedPath + 1] : null;

                if ((leftChild != null) && (leftChild.sliceCount > minBranches)) {
                    n.data.splitVals[deletedPath - 1] = distributeFromLeft(leftChild, affectedChild,
							n.data.splitVals[deletedPath - 1], minBranches);
                    ...
                    n.data.splitVals[deletedPath] = distributeFromRight(rightChild, affectedChild,
							n.data.splitVals[deletedPath], minBranches);
                } else { // Merge with a child sibling.

                    final int holeInx;

                    if (leftChild != null) // Merge with left child.
                        mergeSiblings(leftChild, affectedChild, n.data.splitVals[holeInx = deletedPath - 1]);
                    else // Merge with right child.

                        mergeSiblings(affectedChild, rightChild, n.data.splitVals[holeInx = deletedPath]);
     ...
     * appropriate.
     */
    private final static long distributeFromLeft(final Node leftSibling, final Node thisSibling,
												 final long oldSplitVal, final int minBranches) {
        ...
        if (isLeafNode(leftSibling)) {
            ...
            final long returnThis = leftSibling.data.splitVals[leftSibling.sliceCount - distributeNum - 1];
     ...
     * appropriate.
     */
    private final static long distributeFromRight(final Node rightSibling, final Node thisSibling,
												  final long oldSplitVal, final int minBranches) {
        ...
        if (isLeafNode(rightSibling)) {
            ...
            final long returnThis = rightSibling.data.splitVals[distributeNum - 1];
            ...
            for (int i = rightSibling.sliceCount - distributeNum; i < rightSibling.sliceCount; i++)
     ...
     */
    private final static void mergeSiblings(final Node leftSibling, final Node rightSibling, final long splitValue) {
        if (isLeafNode(leftSibling)) {
     ...
     *        +---+---+---+---+---+---+---+---+---+---+
     */
    private final static void fillHole(final int holeInx, final long[] arr, final int newLen) {
     ...
     */
    public final int count(final long x) {
     ...
     * minBound <= x <= maxBound.
     */
    private final static int count(final Node n, final long x, final long minBound, final long maxBound) {
        int count = 0;
     ...
     * @throws IllegalArgumentException if xMin is greater than xMax.
     */
    public final LongEnumerator searchRange(final long xMin, final long xMax, final boolean reverseOrder) {
        if (xMin > xMax) throw new IllegalArgumentException("xMin is greater than xMax");
        ...
        if (reverseOrder) return new DescendingEnumerator(totalCount, nodeStack, xMax);
     ...
     * call to this method.
     */
    private final static int searchRange(final Node n, final NodeStack nodeStack, final long xMin, final long xMax,
										 final long minBound, final long maxBound, final boolean reverseOrder) {
        int count = 0;
            ...
            if (isLeafNode(n)) {
                ...
                if (reverseOrder) {
                    ...
                    for (int i = 0; i < n.sliceCount; i++) {
                        ...
                        if (currentMax >= xMin) {
                            count += searchRange(n.data.children[i], nodeStack, xMin, xMax, currentMin, currentMax,
									reverseOrder);
                    ...
                    for (int i = n.sliceCount - 2; i >= -1; i--) {
                        ...
                        if (currentMin <= xMax) {
                            count += searchRange(n.data.children[i + 1], nodeStack, xMin, xMax, currentMin,
									currentMax, reverseOrder);
    ...
    private final static class InternalNodeData implements java.io.Serializable {
        ...
        private final long[] splitVals;
        private final Node[] children;

        private InternalNodeData(final int maxBranches) {
            splitVals = new long[maxBranches - 1];
    ...
    private final static class NodeStack {
        ...
        private final void push(final Node value) {
            try {
                ...
                final int newStackSize = (int) Math.min((long) Integer.MAX_VALUE, (((long) stack.length) * 2L) + 1L);

                if (newStackSize == stack.length) throw new IllegalStateException("cannot allocate large enough array");
    ...
    private final static class AscendingEnumerator implements LongEnumerator {
        ...
        private AscendingEnumerator(final int totalCount, final NodeStack nodeStack, final long xMin) {
            count = totalCount;
            stack = nodeStack;
            this.xMin = xMin;
            computeNextLeafNode();
        ...
        public final long nextLong() {
            long returnThis = 0; // To keep compiler from complaining.
        ...
        private final void computeNextLeafNode() {
            ...
            while (true) {
                ...
                if (isLeafNode(next.data.children[0])) wholeLeafNodes += next.sliceCount;
            }
        }
    }

    private final static class DescendingEnumerator implements LongEnumerator {
        private int wholeLeafNodes = 0; // Whole leaf nodes on stack.
        private int count;
        private final NodeStack stack;
        ...
        private DescendingEnumerator(final int totalCount, final NodeStack nodeStack, final long xMax) {
            count = totalCount;
            stack = nodeStack;
            this.xMax = xMax;
            computeNextLeafNode();
