 ...
 * class versioning.
 */
public final class LongHash implements java.io.Serializable {
    private final static long serialVersionUID = 1213745949547L;
    private static final int[] PRIMES = {11, 23, 53, 113, 251, 509, 1019, 2039, 4079, 8179, 16369, 32749, 65521,
			131063, 262133, 524269, 1048571, 2097143, 4194287, 8388587, 16777183, 33554393, 67108837, 134217689,
			268435399, 536870879, 1073741789, 2147483647};
     ...
     */
    public final long put(final long value) {
        ...
        try {
            for (index = (int) (value % (long) (((~value) >>> 63) * m_size)); (m_arr[index] >= 0) && (m_arr[index] != value); index = (index + incr) % m_size) {
                ...
                if (incr == 0) {
                    incr = 1 + (int) (value % (long) (m_size - 1));
                }
            }
        ...
        final long returnVal = m_arr[index];
        m_arr[index] = value;
        m_elements += (int) (returnVal >>> 63);
     ...
     * @throws IllegalArgumentException if value is negative.
     */
    public final long get(final long value) {
        int incr = 0;
        int index;
    ...
    private final void incrSize() {
        ...
        for (int i = 0; i < m_elements; i++) {
            ...
            for (newIndex = (int) (m_dump[oldIndex] % (long) m_size); m_arr[newIndex] >= 0; newIndex =
					(newIndex + incr) % m_size) {
                // Caching increment, which is an expensive operation, at the expense
                // of having an if statement.  I don't want to compute the increment
                // before this 'for' loop in case we get an immediate hit.
                if (incr == 0) {
                    incr = 1 + (int) (m_dump[oldIndex] % (long) (m_size - 1));
                }
            }
