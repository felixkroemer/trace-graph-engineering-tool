package com.felixkroemer.trace_graph_engineering_tool.renderer.ding.impl.cyannotator.utils;

import com.google.common.base.Objects;
import com.felixkroemer.trace_graph_engineering_tool.renderer.ding.internal.util.ColorUtil;
import com.felixkroemer.trace_graph_engineering_tool.renderer.ding.internal.util.MathUtil;
...
public class MultipleGradientEditor extends JPanel {
     ...
     */
    public MultipleGradientEditor(double angle, float[] fractions, Color[] colors, String targetId,
     ...
     */
    public MultipleGradientEditor(Point2D centerPoint, float[] fractions, Color[] colors, String targetId,
								  CyServiceRegistrar serviceRegistrar) {
    ...
    private MultipleGradientEditor(GradientType type, float[] fractions, Color[] colors, String targetId,
								   CyServiceRegistrar serviceRegistrar) {
        this.type = type;
        ...
        if (lastPalette != null) setCurrentPalette(lastPalette);
    }
    ...
    private void init() {
        ...
        layout.setHorizontalGroup(layout.createParallelGroup(Alignment.CENTER, true).addGroup(layout.createSequentialGroup().addGap(10, 10, Short.MAX_VALUE).addComponent(getLinearToggle()).addComponent(getRadialToggle()).addGap(10, 10, Short.MAX_VALUE)).addGroup(layout.createSequentialGroup().addGap(5).addComponent(getPaletteBtn()).addGap(10, 10, Short.MAX_VALUE).addComponent(getReverseBtn()).addGap(5)).addComponent(getGrEditor(), DEFAULT_SIZE, DEFAULT_SIZE, Short.MAX_VALUE).addGroup(layout.createSequentialGroup().addComponent(getAddBtn()).addPreferredGap(ComponentPlacement.RELATED).addComponent(getRemoveBtn()).addPreferredGap(ComponentPlacement.RELATED).addComponent(getEditBtn())).addComponent(getLinearOptionsPnl()).addComponent(getRadialOptionsPnl()));
        layout.setVerticalGroup(layout.createSequentialGroup().addGroup(layout.createParallelGroup(Alignment.CENTER,
				false).addComponent(getLinearToggle(), PREFERRED_SIZE, DEFAULT_SIZE, PREFERRED_SIZE).addComponent(getRadialToggle(), PREFERRED_SIZE, DEFAULT_SIZE, PREFERRED_SIZE)).addPreferredGap(ComponentPlacement.UNRELATED).addGroup(layout.createParallelGroup(Alignment.CENTER, false).addComponent(getPaletteBtn(), PREFERRED_SIZE, DEFAULT_SIZE, PREFERRED_SIZE).addComponent(getReverseBtn(), PREFERRED_SIZE, DEFAULT_SIZE, PREFERRED_SIZE)).addPreferredGap(ComponentPlacement.RELATED).addComponent(getGrEditor(), PREFERRED_SIZE, DEFAULT_SIZE, PREFERRED_SIZE).addPreferredGap(ComponentPlacement.RELATED).addGroup(layout.createParallelGroup(Alignment.CENTER, false).addComponent(getAddBtn(), PREFERRED_SIZE, DEFAULT_SIZE, PREFERRED_SIZE).addComponent(getRemoveBtn(), PREFERRED_SIZE, DEFAULT_SIZE, PREFERRED_SIZE).addComponent(getEditBtn(), PREFERRED_SIZE, DEFAULT_SIZE, PREFERRED_SIZE)).addPreferredGap(ComponentPlacement.UNRELATED).addComponent(getLinearOptionsPnl()).addComponent(getRadialOptionsPnl()));
        ...
        if (isAquaLAF()) {
            ...
            for (int i = 0; i < toggleBtns.length; i++) {
                ...
                if (i == 0) btn.putClientProperty("JButton.segmentPosition", "first");
                else if (i == toggleBtns.length - 1) btn.putClientProperty("JButton.segmentPosition", "last");
                else btn.putClientProperty("JButton.segmentPosition", "middle");
            }
        ...
        if (type == GradientType.LINEAR) typeGroup.setSelected(getLinearToggle().getModel(), true);
        else typeGroup.setSelected(getRadialToggle().getModel(), true);
    ...
    private JToggleButton getLinearToggle() {
        if (linearToggle == null) {
            ...
            linearToggle.addActionListener(evt -> {
                if (linearToggle.isSelected()) type = GradientType.LINEAR;
    ...
    private JToggleButton getRadialToggle() {
        if (radialToggle == null) {
            ...
            radialToggle.addActionListener(evt -> {
                if (radialToggle.isSelected()) type = GradientType.RADIAL;

                updateOptionPanel();

                if (radialToggle.isSelected()) centerPoint = getPointPicker().getValue();
            });
        }
    ...
    private GradientEditor getGrEditor() {
        if (grEditor == null) {
            ...
            grEditor.addActionListener(evt -> {
                ...
                if (getLinearOptionsPnl().isVisible()) updateAnglePicker();
                else if (getRadialOptionsPnl().isVisible()) updatePointPicker();
            });
    ...
    private JPanel getLinearOptionsPnl() {
        if (linearOptionsPnl == null) {
            ...
            layout.setHorizontalGroup(layout.createParallelGroup(Alignment.CENTER, false).addGroup(layout.createSequentialGroup().addComponent(angleLbl).addComponent(getAngleCmb(), PREFERRED_SIZE, DEFAULT_SIZE, PREFERRED_SIZE)).addComponent(getAnglePicker(), PREFERRED_SIZE, DEFAULT_SIZE, PREFERRED_SIZE));
            layout.setVerticalGroup(layout.createSequentialGroup().addGroup(layout.createParallelGroup(Alignment.CENTER, true).addComponent(angleLbl).addComponent(getAngleCmb(), PREFERRED_SIZE, DEFAULT_SIZE, PREFERRED_SIZE)).addPreferredGap(ComponentPlacement.RELATED).addComponent(getAnglePicker(), PREFERRED_SIZE, DEFAULT_SIZE, PREFERRED_SIZE));
    ...
    private void setColors(Color[] colors) {
        var controlPoints = new ArrayList<GradientEditor.ControlPoint>();
    ...
    private void updateOptionPanel() {
        ...
        if (getLinearOptionsPnl().isVisible()) updateAnglePicker();
        if (getRadialOptionsPnl().isVisible()) updatePointPicker();
    }
    ...
    private void updatePointButtons() {
        ...
        getRemoveBtn().setEnabled(selected != null && !Objects.equal(selected, controlPoints.get(0)) && !Objects.equal(selected, controlPoints.get(controlPoints.size() - 1)));
    }
    ...
    private class ColorIcon implements Icon {
        ...
        public void paintIcon(Component c, Graphics g, int x, int y) {
            ...
            g.setColor(c.isEnabled() ? ColorUtil.getContrastingColor(c.getBackground()) : UIManager.getColor("Button" +
					".disabledForeground"));
            g.drawRect(x, y, w, h);
