...
class PopupMenuHelper {
    ...
    void createEdgeViewMenu(View<CyEdge> edgeView, int x, int y, String action) {
        ...
        if (edgeView != null) {
            ...
            if (action.equalsIgnoreCase("OPEN")) {
                ...
                while (i.hasNext()) {
                    if (!i.next().isReady(edgeView, re.getViewModel())) i.remove();
                }
            }
            ...
            if (action.equalsIgnoreCase("OPEN") && menuItemCount == 1 && tfCount == 1) {
                ...
                serviceRegistrar.getService(DialogTaskManager.class).execute(tf.createTaskIterator(edgeView,
                        re.getViewModel()));
            } else {
                var edgeLabel = re.getViewModel().getModel().getRow(edgeView.getModel()).get(CyEdge.INTERACTION,
                        String.class);
                ...
                for (var evtf : usableTFs) {
                    ...
                    addMenuItem(edgeView, menu, provisioner, context, tracker,
                            vtfl.getEdgeViewTaskFactoryMap().get(evtf));
                }
                ...
                for (var edgeCMF : usableCMFs) {
                    ...
                    try {
                        try {
                            ...
                            addCyMenuItem(edgeView, menu, menuItem, tracker,
                                    vtfl.getCyEdgeViewContextMenuFactoryMap().get(edgeCMF));
    ...
    void createNodeViewMenu(View<CyNode> nodeView, int x, int y, String action) {
        ...
        if (nodeView != null) {
            ...
            if (action.equalsIgnoreCase("NEW")) {
                ...
                while (i.hasNext()) {
                    if (!i.next().isReady(nodeView, re.getViewModel())) i.remove();
                }
            }
            ...
            if ((action.equalsIgnoreCase("OPEN") || action.equalsIgnoreCase("Edge")) && menuItemCount == 1 && tfCount == 1) {
                var tf = usableTFs.iterator().next();
                serviceRegistrar.getService(DialogTaskManager.class).execute(tf.createTaskIterator(nodeView,
                        re.getViewModel()));
            } else {
                var nodeLabel = re.getViewModel().getModel().getRow(nodeView.getModel()).get(CyNetwork.NAME,
                        String.class);
                ...
                for (var nvtf : usableTFs) {
                    ...
                    addMenuItem(nodeView, menu, provisioner, context, tracker,
                            vtfl.getNodeViewTaskFactoryMap().get(nvtf));
                }
                ...
                for (var nodeCMF : usableCMFs) {
                    ...
                    try {
                        try {
                            ...
                            addCyMenuItem(nodeView, menu, menuItem, tracker,
                                    vtfl.getCyNodeViewContextMenuFactoryMap().get(nodeCMF));
                        } catch (IllegalArgumentException e) {
                        }
                    } catch (Throwable t) {
                        logger.error("Could not display context menu.", t);
    ...
    void createNetworkViewMenu(Point rawPt, Point xformPt, String action) {
        ...
        if (action.equalsIgnoreCase("OPEN")) {
            ...
            while (i.hasNext()) {
                if (!i.next().isReady(graphView)) i.remove();
            }

            var i2 = usableTFs2.iterator();

            while (i2.hasNext()) {
                if (!i2.next().isReady(graphView, rawPt, xformPt)) i2.remove();
            }
        }
        ...
        if (action.equalsIgnoreCase("OPEN") && menuItemCount == 1 && tfCount == 1) {
            ...
            for (var nvltf : usableTFs2) {
                ...
                addMenuItem(null, menu, provisioner, null, tracker,
                        vtfl.getNetworkViewLocationTaskFactoryMap().get(nvltf));
            }
            ...
            for (var netVMF : usableCMFs) {
                try {
                    try {
                        ...
                        addCyMenuItem(graphView, menu, menuItem, tracker,
                                vtfl.getCyNetworkViewContextMenuFactoryMap().get(netVMF));
                        //TODO: check why this is not logged when using the regular renderer
                    } catch (IllegalArgumentException e) {
                    }
                } catch (Throwable t) {
                    logger.error("Could not display context menu.", t);
    ...
    private void addMenuItem(View<?> view, JPopupMenu popup, NamedTaskFactory tf, Object tunableContext,
        ...
        if (pref == null) {
            if (prefAction != null && prefAction.equalsIgnoreCase("OPEN")) pref = ".";
            else pref = APPS_MENU;
        }
        ...
        if (title == null) { // No title
            ...
            if (last > 0) {
                ...
                if (APPS_MENU.equals(title)) return;

                var gravityTracker = tracker.getGravityTracker(pref);
                var item = createMenuItem(tf, title, toolTip, icon);

                if (insertSepBefore) gravityTracker.addMenuSeparator(gravity - .0001);

                gravityTracker.addMenuItem(item, gravity);

                if (insertSepAfter) gravityTracker.addMenuSeparator(gravity + .0001);
            } else {
     ...
     */
    private void addCyMenuItem(View<?> view, JPopupMenu popup, CyMenuItem menuItem, JMenuTracker tracker,
                               Map<?, ?> props) {
        String pref = null;

        if (props != null) pref = (String) (props.get(PREFERRED_MENU));

        if (pref == null) pref = APPS_MENU;
    ...
    private <T> Collection<T> getPreferredActions(Map<T, Map<String, String>> tfs, String action) {
        ...
        if (action == null) return tfs.keySet();
        ...
        for (var evtf : tfs.keySet()) {
            ...
            if (prefAction == null) prefAction = "NEW";
            if (action.equalsIgnoreCase(prefAction)) usableTFs.add(evtf);
        }
            ...
            if (comp instanceof JMenu) {
                ...
                for (var mc : menu.getMenuComponents()) {
                    if (mc instanceof JSeparator) {
                        ...
                        if (hasSeparator || i == 0 || i == menu.getItemCount() - 1) mc.setVisible(false);
                ...
                if (!hasMenuItem) comp.setEnabled(false);
            }
        }
    }
