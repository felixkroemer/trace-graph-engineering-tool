...
import com.felixkroemer.trace_graph_engineering_tool.renderer.ding.impl.cyannotator.annotations.AnchorLocation;
import com.felixkroemer.trace_graph_engineering_tool.renderer.ding.impl.cyannotator.annotations.DingAnnotation;
import com.felixkroemer.trace_graph_engineering_tool.renderer.ding.impl.cyannotator.create.AbstractDingAnnotationFactory;
import com.felixkroemer.trace_graph_engineering_tool.renderer.ding.impl.cyannotator.tasks.EditAnnotationTaskFactory;
import com.felixkroemer.trace_graph_engineering_tool.renderer.ding.impl.cyannotator.utils.ViewUtils;
import com.felixkroemer.trace_graph_engineering_tool.renderer.ding.impl.undo.AnnotationEdit;
import com.felixkroemer.trace_graph_engineering_tool.renderer.ding.impl.undo.CompositeCyEdit;
import com.felixkroemer.trace_graph_engineering_tool.renderer.ding.impl.undo.LabelEdit;
import com.felixkroemer.trace_graph_engineering_tool.renderer.ding.impl.undo.ViewChangeEdit;
...
import static com.felixkroemer.trace_graph_engineering_tool.renderer.ding.internal.util.ViewUtil.*;
...
public class InputHandlerGlassPane extends JComponent implements CyDisposable {
    ...
    public void cancelClickToAddAnnotation(AnnotationFactory<? extends Annotation> factory) {
        ...
        if (listener != null && factory.equals(listener.getAnnotationFactory())) listener.cancel();
    }
    ...
    private class CanvasKeyListener extends MouseAdapter implements KeyListener {
        ...
        private void endEdit() {
            ...
            if (labelEdit != null) {
                ...
                if (moveAnnotationsEdit != null) moveAnnotationsEdit.saveNewAnnotations();
                if (moveNodesEdit != null) moveNodesEdit.saveNewPositions();
                if (panEdit != null) panEdit.saveNewPositions();
        ...
        public void keyPressed(KeyEvent e) {
            ...
            if (allChanged) re.updateView(DRenderingEngine.UpdateType.ALL_FULL);
            else if (annotationsChanged) re.updateView(DRenderingEngine.UpdateType.JUST_ANNOTATIONS);

            if (moved) resetMoveTimer();
        }
        ...
        private void panCanvas(KeyEvent k) {
            ...
            switch (k.getKeyCode()) {
                case VK_UP:
                    panner.continuePan(0, move);
                    break;
                case VK_DOWN:
                    panner.continuePan(0, -move);
                    break;
                case VK_LEFT:
                    panner.continuePan(move, 0);
                    break;
                case VK_RIGHT:
                    panner.continuePan(-move, 0);
                    break;
        ...
        private boolean moveLabels(KeyEvent k) {
            ...
            if (labelEdit == null) {
                int size = 0;
                if (selectedNodeLabels != null && selectedNodeLabels.size() > 0) size += selectedNodeLabels.size();
                if (selectedEdgeLabels != null && selectedEdgeLabels.size() > 0) size += selectedEdgeLabels.size();
            ...
            else pos = selectedEdgeLabels.iterator().next().getPosition();
        ...
        private void deleteSelected() {
            var tasks = new TaskIterator();
            {
                ...
                if (f.isReady(re.getViewModel())) tasks.append(f.createTaskIterator(re.getViewModel()));
            }
            {
    ...
    private class CanvasMouseWheelListener implements MouseWheelListener {
        ...
        public void mouseWheelMoved(MouseWheelEvent e) {
            ...
            re.updateView(DRenderingEngine.UpdateType.ALL_FULL);
        }
    }
    ...
    private class SelectionClickAndDragListener extends MouseAdapter {
        ...
        private boolean mousePressedCheckHit(MouseEvent e) {
            ...
            if (annotationSelectionEnabled()) {
                ...
                DingAnnotation annotation = picker.getAnnotationAt(DingAnnotation.CanvasID.FOREGROUND, e.getPoint());
                if (annotation != null) {
            ...
            if (labelSelectionEnabled()) {
                ...
                if (selectedLabel == null) selectedLabel = picker.getEdgeLabelAt(e.getPoint());
            ...
            if (selectEdges) {
                ...
                if (edge != null) {
                    if (e.isAltDown() && isLODEnabled(RenderDetailFlags.LOD_EDGE_ANCHORS)) addHandle(edge, e);
            ...
            if (annotationSelectionEnabled()) {
                DingAnnotation annotation = picker.getAnnotationAt(DingAnnotation.CanvasID.BACKGROUND, e.getPoint());
                if (annotation != null) {
        ...
        private Toggle mousePressedHandleAnnotation(DingAnnotation annotation, MouseEvent e) {
            ...
            if (toggle != Toggle.NOCHANGE) re.updateView(DRenderingEngine.UpdateType.JUST_ANNOTATIONS);
        ...
        public void mouseReleased(MouseEvent e) {
            ...
            if (annotationMovingEdit != null || moveNodesEdit != null) {
                if (annotationMovingEdit != null) annotationMovingEdit.saveNewAnnotations();
                if (moveNodesEdit != null) moveNodesEdit.saveNewPositions();
        ...
        public void mouseDragged(MouseEvent e) {
            ...
            if (!labelSelectionManager.isEmpty()) {
                ...
                labelSelectionManager.setCurrentDragPoint(e.getPoint());
                re.updateView(DRenderingEngine.UpdateType.ALL_FAST);
                return;
            }
            ...
            if (!annotationSelection.isEmpty()) {
                ...
                if (annotationSelection.isRotating()) {
                    ...
                    if (annotationMovingEdit == null) annotationMovingEdit = new AnnotationEdit("Move Annotation", re);
            ...
            if (!selectedNodes.isEmpty() || re.getBendStore().areHandlesSelected())
                re.updateView(DRenderingEngine.UpdateType.ALL_FAST);
            else re.updateView(DRenderingEngine.UpdateType.JUST_ANNOTATIONS);
        }
    ...
    private class AddAnnotationListener extends MouseAdapter {
        ...
        public void cancel() {
            if (annotationFactory != null) cyAnnotator.fireAnnotations(); // tell the Annotation panel that we cancelled
        ...
        public void mouseMoved(MouseEvent e) {
            ...
            if (arrow != null) {
                ...
                if (annotations.contains(arrow)) annotations.remove(arrow);
    ...
    private class SelectionRectangleListener extends MouseAdapter {
        ...
        public void mouseReleased(MouseEvent e) {
            if (selectionRect != null) {
                ...
                // Select
                if (!nodes.isEmpty()) select(nodes, CyNode.class, true);
                if (!edges.isEmpty()) select(edges, CyEdge.class, true);
    ...
    private class PanListener extends MouseAdapter {
        ...
        @Override
        public void mouseReleased(MouseEvent e) {
            if (undoPanEdit != null) undoPanEdit.post();
    ...
    private <T extends CyIdentifiable> void toggleSelection(View<T> element, Class<T> type, Toggle toggle) {
        if (element != null) {
            if (toggle == Toggle.SELECT) select(Collections.singletonList(element), type, true);
            else if (toggle == Toggle.DESELECT) select(Collections.singletonList(element), type, false);
        }
    }
