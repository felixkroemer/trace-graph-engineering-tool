...
public class DNodeDetails implements NodeDetails {
    ...
    public Color getColorLowDetail(CyNetworkViewSnapshot netView, View<CyNode> nodeView) {
        if (isSelected(nodeView)) return getSelectedColorLowDetail(netView, nodeView);
        else return getUnselectedColorLowDetail(netView, nodeView);
    }
    ...
    public Paint getFillPaint(View<CyNode> nodeView) {
        if (isSelected(nodeView)) return getSelectedPaint(nodeView);
        else return getUnselectedPaint(nodeView);
    }
    ...
    public static Font computeFont(Font font, Number size, boolean forPdf) {
        ...
        if (size != null) font = font.deriveFont(size.floatValue());
        ...
        if (forPdf) {
            ...
            if (italic && bold) font = font.deriveFont(Font.ITALIC | Font.BOLD);
            else if (italic) font = font.deriveFont(Font.ITALIC);
            else if (bold) font = font.deriveFont(Font.BOLD);
        }
    ...
    private CustomGraphicsInfo getCustomGraphicsInfo(VisualProperty<CyCustomGraphics> cgVP, View<CyNode> node) {
        ...
        if (cg == null || cg == NullCustomGraphics.getNullObject()) return null;
    ...
    public TexturePaint getNestedNetworkTexturePaint(CyNetworkViewSnapshot netView, View<CyNode> nodeView) {
        ...
        try {
            ...
            if (nestedNetworkView != null) {
                ...
                if (nestedRe == null) return null; // not a Ding network
    ...
    public CyNetworkView getNestedNetworkView(CyNetworkViewSnapshot netView, View<CyNode> nodeView) {
        ...
        if (modelNode.getNetworkPointer() == null) return null;
        ...
        if (viewIterator.hasNext()) return viewIterator.next();
