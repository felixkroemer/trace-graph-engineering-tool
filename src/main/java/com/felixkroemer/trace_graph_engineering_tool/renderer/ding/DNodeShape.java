...
public class DNodeShape implements NodeShape {
    ...
    public static final DNodeShape getDShape(final NodeShape shape) {
        if (DEF_SHAPE_MAP.get(shape) == null) return RECTANGLE;
        else return DEF_SHAPE_MAP.get(shape);
    }

    public static final DNodeShape getDShape(final LabelBackgroundShape shape) {
        if (shape == LabelBackgroundShapeVisualProperty.RECTANGLE) return RECTANGLE;
        if (shape == LabelBackgroundShapeVisualProperty.ROUND_RECTANGLE) return ROUND_RECTANGLE;
        return null;
    }
