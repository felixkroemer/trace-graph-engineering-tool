...
import com.felixkroemer.trace_graph_engineering_tool.renderer.ding.debug.DebugProgressMonitorFactory;
...
import com.felixkroemer.trace_graph_engineering_tool.renderer.ding.impl.TransformChangeListener;
...
import static com.felixkroemer.trace_graph_engineering_tool.renderer.ding.internal.util.ViewUtil.invokeOnEDTAndWait;
...
public abstract class RenderComponent extends JComponent {
    ...
    private Snapshot slowCanvasLastPaintSnapshot;
    private Snapshot fastCanvasPanStartedSnapshot;
    private Snapshot fastCanvasLastPaintSnapshot;


    public RenderComponent(DRenderingEngine re, GraphLOD lod) {
        this.re = re;

        // MKTODO Using a size of 1 is a hack. We don't know what the size of the buffer should be until setBounds()
        // is called.
    ...
    abstract com.felixkroemer.trace_graph_engineering_tool.renderer.ding.impl.work.ProgressMonitor getSlowProgressMonitor();
    ...
    private PaintParameters getFastCanvasPaintParams() {
        ...
        if (flags.has(RenderDetailFlags.OPT_EDGE_BUFF_PAN)) { // enable edge buffer panning optimization
            if (flags.renderEdges() != RenderEdges.NONE) {
                return PaintParameters.updateType(updateType); // Fast canvas can render its own edges, don't need to
                // buffer pan
            }
    ...
    public void paint(Graphics g) {
        ...
        if (scaleX != dpiScaleFactor) {
            ...
            if (slowFuture != null) slowFuture.cancel();
            if (fastFuture != null) fastFuture.cancel();
    ...
    private com.felixkroemer.trace_graph_engineering_tool.renderer.ding.impl.work.ProgressMonitor debugPm(UpdateType updateType) {
        return debugPm(updateType, null);
    }

    private com.felixkroemer.trace_graph_engineering_tool.renderer.ding.impl.work.ProgressMonitor debugPm(UpdateType updateType, ProgressMonitor pm) {
