...
import com.felixkroemer.trace_graph_engineering_tool.renderer.ding.PrintLOD;
...
import com.felixkroemer.trace_graph_engineering_tool.renderer.ding.impl.canvas.CompositeGraphicsCanvas;
import com.felixkroemer.trace_graph_engineering_tool.renderer.ding.impl.canvas.MainRenderComponent;
import com.felixkroemer.trace_graph_engineering_tool.renderer.ding.impl.canvas.NetworkImageBuffer;
...
public class DRenderingEngine implements RenderingEngine<CyNetwork>, Printable, CyNetworkViewListener {
    ...
    private boolean updateModel() {
        ...
        if (snapshot == null) return false; // Should happen very infrequently, try again on the next frame.
    ...
    private void handleFit(boolean justSelectedNodes) {
        ...
        synchronized (dingLock) {
            ...
            if (netViewSnapshot == null || netViewSnapshot.getNodeCount() == 0) return;

            NetworkTransform transform = renderComponent.getTransform();
            if (transform.getWidth() == 0 || transform.getHeight() == 0) return;
            ...
            if (flags.all(LOD_HIGH_DETAIL, LOD_NODE_LABELS, OPT_LABEL_CACHE)) {
                ...
                for (View<CyNode> node : nodeIterable) {
                    ...
                    if (label != null) {
                        ...
                        if (bounds.getMinX() < extents[0]) extents[0] = bounds.getMinX();
                        if (bounds.getMinY() < extents[1]) extents[1] = bounds.getMinY();
                        if (bounds.getMaxX() > extents[2]) extents[2] = bounds.getMaxX();
                        if (bounds.getMaxY() > extents[3]) extents[3] = bounds.getMaxY();
                    }
                }
            }
    ...
    public void zoomToPointer(int ticks, int imageX, int imageY) {
        if (getViewModelSnapshot().isValueLocked(BasicVisualLexicon.NETWORK_SCALE_FACTOR)) return;
    ...
    public class Panner {
        ...
        public void continuePan(double dx, double dy) {
            if (!panned) renderComponent.startPan();
