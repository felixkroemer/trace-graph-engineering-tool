	...
	public void addAnnotations(Collection<? extends Annotation> annotations) {
		...
		invokeOnEDTAndWait(() -> {
			annotationsByView.forEach((view, annotationsByCanvas) -> {
				...
				if (annotationsByCanvas.containsKey(DingAnnotation.CanvasID.FOREGROUND)) {
					all.addAll(annotationsByCanvas.get(DingAnnotation.CanvasID.FOREGROUND));
				}
				if (annotationsByCanvas.containsKey(DingAnnotation.CanvasID.BACKGROUND)) {
					all.addAll(annotationsByCanvas.get(DingAnnotation.CanvasID.BACKGROUND));
				}
...
//		});
	...
	private Map<CyNetworkView, Map<DingAnnotation.CanvasID, List<DingAnnotation>>> groupByViewAndCanvasAndFlatten(
			Collection<? extends Annotation> annotations,
			boolean incudeExisting
	) {
		var map = new HashMap<CyNetworkView, Map<DingAnnotation.CanvasID, List<DingAnnotation>>>();
